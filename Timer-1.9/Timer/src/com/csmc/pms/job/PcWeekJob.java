...
public class PcWeekJob {
    public static final String module = PcDayJob.class.getName();

    public void start() {
        FileSystemXmlApplicationContext ctx = null;
        
        Log.logDebug("pms-PcWeekJob start[" + new Timestamp(System.currentTimeMillis()) + "]", module);
        ...
        try {
            ...
            pmsService.invokeWeekUnfinishPcService("光刻安全检查");//"scrubber巡检"
            Log.logInfo("End Service--------------", module);
            
        } catch(Exception e) {
            e.printStackTrace();
            Log.logError("pms-PcWeekJob error[" + e.getMessage() + "]", module);
        } finally {
            ctx.close();
            Log.logDebug("pms-PcWeekJob end[" + new Timestamp(System.currentTimeMillis()) + "]", module);
        }
    }
    
    public static void main(String[] args) {
    	PcWeekJob job = new PcWeekJob();
        job.start();
    }
}
