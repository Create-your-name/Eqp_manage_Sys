...
public class PmEventChart extends PmsChartApplet {
	...
	private void createDataset(List dataList, DefaultCategoryDataset barDataset, DefaultCategoryDataset lineDataset) {       
        ...
        for (int i = 0; i < dataList.size(); i++) {
			...
			//column keys...
			barDataset.addValue(Double.parseDouble(dataMap.get("AVGDAY").toString()), seryName1, dataMap.get("PERIOD_NAME").toString());
			barDataset.addValue(Double.parseDouble(dataMap.get("AVGTIME").toString()), seryName2, dataMap.get("PERIOD_NAME").toString());
	
			lineDataset.addValue(Double.parseDouble(dataMap.get("EVENTCOUNT").toString()), seryName3, dataMap.get("PERIOD_NAME").toString());
		}
    }
    ...
    private JFreeChart createChart(List dataList) {
    	...
    	JFreeChart jfreechart = ChartFactory.createBarChart("保养事件统计图", "周期", "平均时数", barDataset, PlotOrientation.VERTICAL, false, true, false);
    ...
    public void init() {
    	...
    	//su="http://localhost:8080/csmcgui/control/getPmEventHist?startDate=2007-08-06&endDate=2007-11-02&equipmentType=PSEM&eqpId=&keyEqp=&adjustEqp=&measureEqp=&dayCount=10";
