...
{
	this.showLine=false;
	this.lineFolder="/pms/";//lПͼƬτ¼�this.Indent=13;
	...
	this.getSelect=function getSelect(){//·µ»ّ¡ձ½ᶄ1�7
...
function Tree_on_action(action)//fire when click node
{
	if(action.type==Tree_LINK)//link
	{
		...
		if( target=="mainFrame"||"_self"||target=="_top"||target=="_parent"||target=="_blank")
			window.open(action.link,target);
		else
		{
			var command = target +".location = action.link";
			if(action.link!="") try{eval(command);}catch(e){alert("½ű¾Ջѐ´\r\n\r\n"+e.description);}
		}
	}
	else if(action.type==Tree_SCRIPT)//run script
	{
		try{eval(action.script);}catch(e){alert("½ű¾Ջѐ´\r\n\r\n"+e.description);}
	}
}
//----------------
...
function Tree_GetLineImg(node)
{
 ...
 %>
 var tree_node=new Array(<%=menuString%>);
...
tree.showLine=true;
tree.Indent=13;
tree.folderImg1="<%=request.getContextPath()%>/treeimg/clsfld.gif";
tree.lineFolder="<%=request.getContextPath()%>/treeimg/";
tree.folderImg2="<%=request.getContextPath()%>/treeimg/openfld.gif";
tree.fileImg="<%=request.getContextPath()%>/treeimg/link.gif";
tree.target="_blank";
