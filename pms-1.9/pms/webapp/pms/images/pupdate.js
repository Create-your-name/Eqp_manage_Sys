...
function moveMonth(dir) {
 ...
 if (obj != null) {
  ...
  else if ((dir.toLowerCase() == "forward")&&(obj.selectedIndex < 60)) {obj.selectedIndex++;}
  else {limit = true;}}
 if (!limit) {
  tmp = obj.options[obj.selectedIndex].value.split("|");
  dptrYear  = tmp[0];
  dptrMonth = tmp[1];
  setCalendar(dptrYear-2,dptrMonth);}
 else {
  if (ppcIE) {
...
function makeCell(bgr,year,month,date) {
 ...
 var evt = "onMouseOver=\"this.style.backgroundColor=\'#FF0000\'\" onMouseOut=\"this.style.backgroundColor=\'"+bgr+"\'\" onMouseDown=\"selectDate("+param+")\" ";
 ...
 if ((ppcNow.getDate() == date)&&(ppcNow.getMonth() == month)&&(getFullYear(ppcNow) == year)) {
  cellValue = "<b><font color=\"blue\">"+cellValue+"</font></b>";}
 var cellCode = "";
 if (date == 0) {
...
function setSelectList(year,month) {
 ...
 while (i < 61) {
  ...
  if ((year==getFullYear(ppcNow))&&(month==ppcNow.getMonth())){
     obj.selectedIndex=i;	
  }
  i++;
  month++;
