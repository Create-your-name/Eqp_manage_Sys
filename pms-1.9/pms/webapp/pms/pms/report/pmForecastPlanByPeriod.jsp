...
<%
	...
	if (null == eDate) {
		//结束时间默认为开始时间3月后
	   	...
	   	eDate = formatter.format(calendar.getTime());
	}
...
<tr>
  <td><fieldset>
      <legend>PM预测报表(默认 3 months，最大不超过 6 months)</legend>
      <table width="100%" border="0" cellspacing="1" cellpadding="2">
	     <tr bgcolor="#DFE1EC">
	       <td width="15%" class="en11pxb">开始日期:</td>
	       <td width="35%">
	       	 <input type="text" id="startDate" name="startDate" value="<%=sDate%>" readonly>
	       </td>

	       <td width="15%" class="en11pxb">结束日期:</td>
	       <td width="35%">
	       	 <input type="text" id="endDate" name="endDate" value="<%=eDate%>" readonly>
	       </td>
	    ...
	    <tr bgcolor="#DFE1EC">
	       ...
	       <td width="35%"><select id="equipmentType" name="equipmentType">
	          <option value=""></option>
		    ...
		    <td width="15%" class="en11pxb">EqpId:</td>
		    <td width="35%"><select id="equipmentId" name="equipmentId">
		          <option value=""></option>
	    	</select></td>
	    ...
	    <tr bgcolor="#DFE1EC">
		    ...
		    <td width="35%"><select id="period" name="period">
		  		<option value=""></option>
		  	</select></td>

			<td width="15%" class="en11pxb">关键设备:</td>
			<td width="35%">
	    ...
	    <tr bgcolor="#DFE1EC">
	    	<td width="15%" class="en11pxb">计量设备:</td>
			<td width="35%">
			...
			<td width="15%" class="en11pxb">校准设备:</td>
			<td width="35%">
  ...
  <td><fieldset>
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="2">
        <tr bgcolor="#ACD5C9">
        	...
        	<td rowspan="2" class="en11pxb">周期名</td>
        	<td rowspan="2" class="en11pxb">基本天数</td>
        	<td rowspan="2" class="en11pxb">标准工时</td>
            <%
              Map monthMap;
              String yearMonth;
              int startDayOfMonth;
    	...
    	if(pmScheduleList != null && pmScheduleList.size() > 0) {
			...
			String eqpId, periodName, defaultDays, standardHour;
			String scheduleDate;
			int countPeriod;
			String title;
       		...
       		for (int k = 0;k < pmScheduleList.size();k++) {
				...
				periodName = (String) map.get("PERIOD_NAME");
				scheduleDate = (String) map.get("SCHEDULE_DATE");
				defaultDays = (String) map.get("DEFAULT_DAYS");
				standardHour = (String) map.get("STANDARD_HOUR");

				if (!tmpEqpId.equals(eqpId) || !tmpPeriodName.equals(periodName)) {
					if (tmpEqpId.equals("")) {
					...
					tmpEqpId = eqpId;
					tmpPeriodName = periodName;

					%>
    					<td class="en11pxb"><%=eqpId%></td>
    					<td class="en11pxb"><%=periodName%></td>
    					<td class="en11pxb"><%=defaultDays%></td>
    					<td class="en11pxb"><%=standardHour%></td>
					<%
				}
				    ...
				    for (j = startDayOfMonth;j <= endDayOfMonth; j++) {
			            ...
			            if (scheduleDate.equals(yearMonth + "/" + dayOfMonth)) {
			            	title = eqpId + " " + scheduleDate + "<br>" + "保养周期 (工时)<br>"
			            			+ periodName + " (" + (String)map.get("STANDARD_HOUR") + ")";
			            	...
			            	while (k < pmScheduleList.size()-1) {
								...
								if (tmpEqpId.equals(eqpId) && tmpPeriodName.equals(periodName)) {
									...
									if (scheduleDate.equals(tmpScheduleDate)) {
										title = title + "<br>" + periodName + " (" + (String)map.get("STANDARD_HOUR") + ")";
										countPeriod++;
									} else {
										break;                              //需比较下一个td日期
									}
