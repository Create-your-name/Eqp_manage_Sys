...
<script lanaguage="javascript">
	...
	function filterEqpTypeChange()
	{
		...
		var actionURL = '<ofbiz:url>/queryEqpIdByValidData</ofbiz:url>?periodIndex='+periodIndex+'&itemIndex='+newItemIndex+'&sDate='+sDate+'&eDate='+eDate;
	...
	return {
		...
		createDataStore : function() {
		    ...
		    periodDS = new Ext.data.Store({
		        proxy: new Ext.data.HttpProxy({url: '<ofbiz:url>/queryPMPeriodByEqpTypeAndDate</ofbiz:url>'}),
			    reader: new Ext.data.JsonReader({}, periodRecordDef)
		    });

		    flowActionItemDS = new Ext.data.Store({
		        proxy: new Ext.data.HttpProxy({url: '<ofbiz:url>/queryFlowActionItemByEqpTypeAndPeriod</ofbiz:url>'}),
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>
      <fieldset><legend>查询机台保养纪录参数分析图</legend>
...
<table border="0" cellspacing="0" cellpadding="0">
  <tr height="30">
    <td><ul class="button">
			<li><a class="button-text" href="javascript:doSubmit('<%=request.getContextPath()%>/control/showPmParamAnalysisChart')"><span>&nbsp;显示&nbsp;</span></a></li>
	</ul></td>
    <td><ul class="button">
