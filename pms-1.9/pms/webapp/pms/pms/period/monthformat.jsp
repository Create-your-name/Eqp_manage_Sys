...
<%@ page import="org.ofbiz.base.util.UtilFormatOut"%>
   ...
   <table border="0" cellpadding="0" bgcolor="<%=colorMap.get("background")%>"  bordercolor="<%=colorMap.get("borderColor")%>">
    ...
    for(j=1; j<=ndays; j++) 
    {
		...
		String timeRangeIndex = UtilFormatOut.checkNull((String)timeRangeMap.get(String.valueOf(firstDay.get(Calendar.YEAR))+String.valueOf(month)+String.valueOf(j)));    	
	...
	%>
        <td align="center" bgcolor="<%=tdcolor%>" class="k">
       		<font color="<%=colorMap.get("fontColor")%>" onmouseover="color='#0000FF'" onmouseout=<%="color='"+ colorMap.get("fontColor")+"'"%> 
       				onclick="daytitle_onclick(this,'<%=firstDay.get(Calendar.YEAR)%>','<%=(month+1)%>','<%=j%>','<%=creator%>','<%=description%>','<%=periodStr%>','<%=isFinished%>','<%=timeRangeIndex%>')">
		        <B><%=j%></B>
        	</font>
		</td>
