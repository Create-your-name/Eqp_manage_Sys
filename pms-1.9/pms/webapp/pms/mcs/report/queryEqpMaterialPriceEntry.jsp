...
List eqpMtrGrpPriceList = (List) request.getAttribute("eqpMtrGrpPriceList");
%>
...
<table border="0" cellspacing="0" cellpadding="0">
  <tr height="30">
    <td><ul class="button">
			<li><a class="button-text" href="javascript:doSubmit('<ofbiz:url>/queryEqpMaterialPriceEntry</ofbiz:url>')"><span>&nbsp;查询&nbsp;</span></a></li>
	</ul></td>
    <td><ul class="button">
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2">
    	<tr bgcolor="#ACD5C9" height="13">
    		...
    		<td class="en11pxb">金额</td>
    	</tr>

		<%
		if(eqpMtrGrpPriceList != null && eqpMtrGrpPriceList.size() > 0) {
			for(Iterator it1 = eqpMtrGrpPriceList.iterator();it1.hasNext();) {
		...
		%>
		        <tr class="tablelist">
        			...
        			<td class="en11px">
        			    <a href="#" onclick="turnto('<ofbiz:url>/queryEqpMaterialPrice?equipmentId=<%=map1.get("USING_OBJECT_ID")%>&mtrGrp=<%=map1.get("MTR_GRP")%></ofbiz:url>')" >
        			        <%=map1.get("TOTAL_PRICE")%>
        			    </a>
        			</td>
    		    </tr>
		<%
