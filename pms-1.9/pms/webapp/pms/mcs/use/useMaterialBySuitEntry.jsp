...
<%
    ...
    String materialSuitIndex = UtilFormatOut.checkNull(request.getParameter("materialSuitIndex"));
%>
...
var useMaterialById = function() {
	...
	return {
		...
		createDataStore : function() {
		    ...
		    suitDS = new Ext.data.Store({
		        proxy: new Ext.data.HttpProxy({url: '<ofbiz:url>/getJsonCommonSuit</ofbiz:url>'}),
			    reader: new Ext.data.JsonReader({}, Ext.data.Record.create([{name: 'materialSuitIndex'}, {name: 'suitName'}]))
		    });
		},
		...
		createCombox : function() {
		    ...
		    mtrGrpCom.on('select', this.loadSuit);

		    //设置套件
		    suitCom = new Ext.form.ComboBox({
			    store: suitDS,
			    displayField:'suitName',
			    valueField:'materialSuitIndex',
			    hiddenName:'materialSuitIndex',
			    typeAhead: true,
			    mode: 'local',
			    width: 170,
			    triggerAction: 'all',
		...
		initLoad : function() {
	    	...
	    	mtrGrpDS.load({callback:function(){ mtrGrpCom.setValue(mtrGrp); if(mtrGrp != ''){useMaterialById.loadSuit();} }});
		},

		loadSuit : function() {
			var val = mtrGrpCom.getValue();
			var materialSuitIndex = '<%=materialSuitIndex%>';
	 		suitDS.load({params:{mtrGrp:val}, callback:function(){ suitCom.setValue(materialSuitIndex); }});
		}
	}
}();
...
function queryEqpMaterial() {
    doSubmit("<ofbiz:url>/useMaterialBySuitEntry</ofbiz:url>");
}
...
function changeMaterialStatus(newStatus, newStatusDesc) {
    Ext.MessageBox.confirm('操作确认', '您确信选择 ' + newStatusDesc + ' 吗？',function result(value){
        if(value=="yes"){
			if (checkForm()) {
				doSubmit("<ofbiz:url>/useMaterialBySuit</ofbiz:url>?newStatus=" + newStatus);
        	}
		}else{
			return;
		}
...
<tr>
  <td><fieldset>
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="2" height="30">
	    ...
	    <tr>
    		<td class="tabletitle" width="12%">
    		    <i18n:message key="mcs.suit_name" />
    		</td>
			<td class="tablelist" width="30%">
			    <input type="text" size="40" name="suitSelect" autocomplete="off"/>
			</td>
			...
			<td class="tablelist" width="30%">
			    <input type="text" class="input" size="20" name="aliasName" value="<ofbiz:field attribute="aliasName"/>" />
			</td>
	    </tr>
	  </table>
  </fieldset></td>
